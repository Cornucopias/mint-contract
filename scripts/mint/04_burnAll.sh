...
BURN_ASSET="-1 ${policy_id}.${ref_name} + -1 ${policy_id}.${nft_name}"
...
alltxin=""
TXIN=$(jq -r --arg alltxin "" --arg policy_id "$policy_id" --arg name "$ref_name" 'to_entries[] | select(.value.value[$policy_id][$name] == 1) | .key | . + $alltxin + " --tx-in"' ../tmp/script_utxo.json)
script_tx_in=${TXIN::-8}
